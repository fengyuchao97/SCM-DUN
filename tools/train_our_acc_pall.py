...
from csi.architectures import SCMamba
...
# 假设你希望使用 GPU 0 和 GPU 1 进行训练
gpu_ids = [3,4]  # 这里指定要使用的 GPU，假设要使用 GPU 0 和 GPU 1
device = torch.device(f"cuda:{gpu_ids[0]}" if torch.cuda.is_available() else "cpu")  # 主设备设为第一个 GPU
print(f"Using device: {device}")

# 初始化 mask 并放置到指定 GPU 上
...
# 使用 DataParallel 包装模型，并指定使用的 GPU
if len(gpu_ids) > 1:
    print(f"Using GPUs: {gpu_ids} for training...")
    model = nn.DataParallel(model, device_ids=gpu_ids, output_device=gpu_ids[0])  # 指定多卡训练的 GPU
...
def main():
    ...
    for epoch in range(start_epoch+1, cfg.OPTIMIZER.MAX_EPOCH):
        ...
        #     # sio.savemat(os.path.join(output_dir, "test"), {"pred": test_out})
        # else:
        #     if sam_mean < sam_min:
        #         sam_min = sam_mean
        #         checkpoint(model, ema, optimizer, scheduler, epoch, output_dir, logger)
